...
export default new class {
    ...
    if (user.isLogin) {
      const sma = {
        ...
        origin: function (options) {
          ...
          for (const ck of this.cookies.cookies) {
          ...
          if (options.method === 'post') {
            // 检查post请求，全局插入CSRF
            for (const t of this.cookies.cookies) {
              if (t.name === 'bili_jct') {
  ...
  async origin (options) {
    ...
    const s = (new Date()).valueOf() // 获取当前毫秒时间戳
    if (this.thread >= 7) {
      /* 并发限制，不得超过7 */
      await sleep(200)
